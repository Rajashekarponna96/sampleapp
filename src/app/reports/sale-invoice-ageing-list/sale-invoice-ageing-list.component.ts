...
import { SaleinvoiceAgeingListService } from 'src/app/services/saleinvoice-ageing-list.service';
import { ReplaceZeroWithEmptyForCSPipe } from 'src/app/pipes/replace-zero-with-empty-for-cs.pipe';

@Component({
  selector: 'app-sale-invoice-ageing-list',
  templateUrl: './sale-invoice-ageing-list.component.html',
  styleUrls: ['./sale-invoice-ageing-list.component.css'],
  providers: [ConfirmationService,MessageService,ReplaceZeroWithEmptyForCSPipe]
})
export class SaleInvoiceAgeingListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.exportColumns = [
      ...
      { title: 'Value', dataKey: 'debit'},
      { title: 'Received', dataKey: 'received'},
      { title: 'Receivable', dataKey: 'receivable'},
  ...
  loadSaleInvoiceAgeingList(offset:number) { 
    ...
    let ahlService:SaleinvoiceAgeingListService = new SaleinvoiceAgeingListService(this.httpClient)
    let criteria:any = {offset:offset};
    console.log('CRITERIA',criteria)
    this._invSub = ahlService.fetchSaleInvoiceAgeingList(criteria).subscribe({
  ...
  newExportPdf() {
    ...
    let b = 'Sale Invoice Ageing List as on ' + datePipe.transform(this.today,'fullDate');
    ...
    let formattedBody = glpdf.map((item:any) => {
      ...
      item.receivable = this.replZero.transform(item.receivable,'')
      item.received = this.replZero.transform(item.received,'')
      return item;
    });
    ...
    autoTable(doc,{
      ...
      columnStyles: {
        debit: { halign: 'right' },
        received: { halign: 'right' },
        receivable: {halign: 'right'}
        }
    });
  ...
  getTotalAmount() {
    ...
    for (let index = 0; index < this.invoiceList.length; index++) {
      ...
      totalAmount = totalAmount + parseFloat(element.receivable)
    }
    return totalAmount
  }
